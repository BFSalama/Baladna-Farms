...
class ChangeEffectiveWizard(models.TransientModel):
    ...
    def resequence_stj(self):
        entries = self.env['account.move'].sudo().search([('sequence_prefix', 'ilike', "stj")])
        if entries.ids :
            self.env['account.resequence.wizard'].sudo().create({
                'move_ids': entries.ids,
                'first_name': 'STJ/2022/01/0001',
            }).resequence()
        else:
            raise UserError("No inventory entries found .")
    ...
    def validate_items_date(self):
        ...
        if entries.ids:
            for rec in entries:
                do_update(query.update_journal_line_date_by_id, rec.date, rec.id)
