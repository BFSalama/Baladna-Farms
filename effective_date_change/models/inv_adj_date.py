...
class InventoryAdjustmentDate(models.Model):
    ...
    def _apply_inventory(self):
        move_vals = []
        if hasattr(self, 'inventory_adjustment_name'):
            inventory_adjustment_name = self.inventory_adjustment_name
            self = self.quant_ids
        ...
        for quant in self:
            ...
            if float_compare(quant.inventory_diff_quantity, 0, precision_rounding=quant.product_uom_id.rounding) > 0:
                try:
                    move_vals.append(
                        quant.with_context(inventory_name=inventory_adjustment_name)._get_inventory_move_values(quant.inventory_diff_quantity,
...
class InheritApplyToAll(models.TransientModel):
    ...
    def action_apply(self):
        if len(self.quant_ids.ids) == 1:
            if hasattr(self, 'inventory_adjustment_name'):
                InventoryAdjustmentDate._apply_inventory(self)
            else:
